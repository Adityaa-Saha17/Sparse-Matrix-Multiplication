...
void spmm_csr_tiled_v3_kernel(int M, int N,
    ...
    // Per-warp shmem stage for the row's nnz: identical layout and size to
    // spmm_tiled / spmm_tiled_v2. 8 warps * 64 entries * (int + float) = 4 KB/block.
    ...
    for (int col_tile = 0; col_tile < N; col_tile += COL_TILE) {
        // Register tile: 4 partial sums per lane, covering cols
        // {col_tile + lane + 0, +32, +64, +96}.
        ...
        for (int p_base = row_start; p_base < row_end; p_base += NNZ_TILE) {
            ...
            // Consume the staged tile. Scalar lane-strided B loads: each
            // lane issues 4 independent 4-byte loads at col offsets
            // {lane, lane+32, lane+64, lane+96}. Each stride is a single
            // coalesced 128-byte segment (32 lanes * 4 B). The compiler
            // can pipeline the 4 loads with the FMAs since the addresses
            // are independent, which is the SM-throughput-preserving
            // alternative to the float4 burst used in spmm_tiled_v2.
        ...
        // Write back this row's col-tile slice. Four coalesced scalar
        // stores per warp (one per stride).
        #pragma unroll
        for (int i = 0; i < COLS_PER_LANE; ++i) {
            const int col_off = lane + i * 32;
...
void spmm_tiled_v3(const CSR& d_A, const float* d_B, float* d_C, int N) {
    ...
    spmm_csr_tiled_v3_kernel<<<grid, block>>>(d_A.num_rows, N,
